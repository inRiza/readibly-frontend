...
export default function TypeTemplatesPage() {
  const router = useRouter();

  const handleTemplateSelect = (template: Template) => {
    router.push(`/learn/type?template=${template.id}`);
  };
  ...
  return (
    <DashboardLayout>
      ...
      >
        ...
        >
          <div>
            <Link href="/learn" className="flex items-center text-gray-600 hover:text-gray-900 mb-2">
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back to Learning Center
            </Link>
            <h1 className="text-3xl font-bold">Typing Templates</h1>
            <p className="text-gray-600 mt-2">Choose a template to practice typing</p>
          </div>
        </motion.div>
